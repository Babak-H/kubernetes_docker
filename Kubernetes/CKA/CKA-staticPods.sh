...
# it might take 1-2 minutes until pod is automatically created
...
k get po -A
# if a pod's "ownReferences.kind" has kind "NODE" it means that its static pod
...
k get node
k describe node cluster1-controlplane1 | grep Taint -A1 # node-role.kubernetes.io/control-plane:NoSchedule  => for toleration
k get node cluster1-controlplane1 --show-labels # node-role.kubernetes.io/control-plane: "" => for nodeSelector
...
spec:
  ...
  # nodeselector chooses the pod based on the label (key-value)
  nodeSelector:                                
